...
// Antes no existía: cambiar una contraseña obligaba a correr un script contra
// la base. Es el hueco más molesto en la práctica — un cliente que sospecha
// que le vieron la clave tiene que poder reaccionar solo, sin depender de que
// alguien se la cambie a mano.
...
// ============================================================
// Segundo factor (2FA) — opcional, ver totp.js
// ============================================================

// POST /api/auth/2fa/setup — genera el secreto y el QR, sin activarlo todavía.
// El 2FA recién se activa cuando la persona demuestra que puede generar un
// código válido (ver /2fa/activate): si lo activáramos acá y el QR se escaneó
// mal, la cuenta quedaría inaccesible.
router.post('/2fa/setup', authenticateToken, async function(req, res, next) {
  try {
    ...
    const qr = await totp.generarQr(secreto, req.user.username, process.env.STORE_NAME || 'PiezaExpress');
