...
async function authenticateToken(req, res, next) {
  ...
  // El header Authorization se sigue aceptando como alternativa para clientes
  // que no son un navegador (tests, curl): no debilita nada, porque para
  // usarlo hay que tener ya un token válido, y el punto del cambio era que el
  // JavaScript de la página no pueda leerlo.
  ...
  // Corte de sesiones al cambiar la contraseña: se rechaza cualquier token
  // emitido ANTES del último cambio. Sin esto, un token robado seguiría
  // sirviendo hasta vencer (8hs) aunque la víctima cambiara su clave.
  //
  // Cuesta una consulta por request autenticado. Se acepta a conciencia: acá
  // el tráfico es de un puñado de personas administrando clientes, es una
  ...
  try {
    ...
    if (fila.password_changed_at) {
      ...
      // siguiente caían en el mismo segundo. Ver AUDITORIA.md.
      //
      ...
      // robada— usa un token de minutos u horas antes.
...
// 'admin' puede todo; 'editor' consulta clientes pero no los modifica ni ve el
